...
class SST2Dataset:
    ...
    def _tokens_to_tensors(self, tokens_list, max_length):
        ...
        for i, tokens in enumerate(tokens_list):
            for j, token in enumerate(tokens):
                if j >= max_length:
                    break
                tensor[i, j] = self.vocab.stoi.get(token, 0)
    ...
    def _mlp_featurize(self):
        ...
        val_tensors = self._tokens_to_tensors(
            [self.vocab._tokenize(text) for text in self.val_data], max_length
        )
        dev_tensors = self._tokens_to_tensors(
            [self.vocab._tokenize(text) for text in self.dev_data], max_length
        )
        test_tensors = self._tokens_to_tensors(
            [self.vocab._tokenize(text) for text in self.test_data], max_length
        )

        return train_tensors, val_tensors, dev_tensors, test_tensors


def sst2_data_loader(
    train_data_filename: str,
    train_labels_filename: str,
    dev_data_filename: str,
    dev_labels_filename: str,
    ...
    dataset = SST2Dataset(feature_type, feature_config, model_type)
    dataset.load_data(
        train_data_filename,
